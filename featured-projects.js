document.addEventListener('DOMContentLoaded', () => {
    ...
    featuredProjects.forEach(project => {
        ...
        const accent = feat.accentColor || 'var(--lab-palette-primary-main)';
        const icon = feat.icon || 'ph ph-briefcase';
        const outcome = feat.outcome || project.seo?.description || 'Solution outcome detail...';
        ...
        item.innerHTML = `
            <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 25px;">
                <div class="portfolio-icon">
                    <i class="${icon}" style="color: ${accent}; font-size: 3rem; filter: drop-shadow(0 4px 8px ${accent}22);"></i>
                </div>
                <span style="background: ${accent}; color: white; font-size: 0.65rem; padding: 4px 10px; border-radius: 20px; font-weight: 800; letter-spacing: 0.05em; text-transform: uppercase;">Featured</span>
            </div>
            ...
            <div style="flex-grow: 1;">
                ...
                <p style="color: var(--lab-text-secondary); font-size: 0.95rem; line-height: 1.5; margin-bottom: 25px; font-weight: 500; text-align: left;">${outcome}</p>
                
                <div style="display: flex; gap: 10px; flex-wrap: wrap; margin-bottom: 30px; justify-content: flex-start;">
                    ${metrics.map(m => `
                        <div style="background: ${accent}11; border: 1px solid ${accent}22; color: ${accent}; padding: 8px 14px; border-radius: 10px; font-size: 0.8rem; font-weight: 700; display: flex; align-items: center; gap: 8px;">
                            <i class="${m.icon || 'ph ph-check-circle'}" style="font-size: 1rem;"></i> ${m.value || ''}
                        </div>
                    `).join('')}
                </div>
            </div>
        ...
        item.onmouseenter = () => {
            ...
            item.style.boxShadow = `0 15px 35px rgba(0,0,0,0.1), 0 0 15px ${accent}11`;
            item.style.borderColor = accent;
        };
        item.onmouseleave = () => {
            item.style.transform = 'translateY(0)';
            item.style.boxShadow = 'var(--lab-shadow-medium)';
            item.style.borderColor = `${accent}33`;
        };
